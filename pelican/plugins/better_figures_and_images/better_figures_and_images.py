    ...
    for img in soup(['img', 'object']):
        ...
        if src is None:
            ...
            if not src_candidates:
                ...
                logger.debug('Better Fig. Skip src: %s', f'{img_path}/{img_filename}')
                continue
        ...
        try:
            if img.name == 'img':
                im = Image.open(src)
                extra_style = f'width: {im.size[0]}px; height: auto;'
            else:
                svg = pysvg.parser.parse(src)
                extra_style = f'width: {svg.get_width()}px; height: auto;'
        except IOError as e:
        ...
        if img.name == 'img' and img['alt'] == img['src']:
            img['alt'] = ''

        if fig := img.find_parent('div', 'figure'):
            if fig.get('style'):

...
def check_for_thumbnail_directory(pelican_output_path):
	...
	try:
		if not os.path.exists(
			f"{pelican_output_path}/{output_directory_for_thumbnails}"
		): # If the directory doesn't exist already...
			os.makedirs(f"{pelican_output_path}/{output_directory_for_thumbnails}")
			return True
...
def download_thumbnail(video_id_from_shortcode, video_thumbnail_url, video_service_name, pelican_output_path):
	...
	if not os.path.exists(
		f"{pelican_output_path}/{output_directory_for_thumbnails}/{video_service_name}_{video_id_from_shortcode}.jpg"
	): # If the thumbnail doesn't already exist...
		logger.debug(
			f"Video Privacy Enhancer Plugin: Downloading thumbnail from the following url: {video_thumbnail_url}"
		)
		...
		if (len(all_instances_of_the_shortcode) > 0): # If the article/page HAS any shortcodes, go on. Otherwise, don't (to do so would inadvertantly wipe out the output content for that article/page).
			...
			for shortcode_to_parse in all_instances_of_the_shortcode:
				...
				video_id_from_shortcode = re.findall(
					f'(?<={shortcode_to_search_for_not_including_exclamation_point}'
					+ r'\().*?(?=\))',
					shortcode_to_parse,
				)[0]

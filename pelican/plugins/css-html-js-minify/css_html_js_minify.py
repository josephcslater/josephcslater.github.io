...
def main(pelican):
    for f in glob.iglob(f'{pelican.output_path}/**/*.htm*', recursive=True):
        process_single_html_file(f, overwrite=True)
    for f in glob.iglob(f'{pelican.output_path}/**/*.css', recursive=True):
        process_single_css_file(f, overwrite=True)
    for f in glob.iglob(f'{pelican.output_path}/**/*.js', recursive=True):

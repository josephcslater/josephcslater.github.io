...
''' % self.testtitle
    ...
    def testKnitrSettings(self):
        settings = read_settings(
            ...
            override={
                ...
                'RMD_READER_KNITR_OPTS_CHUNK': {'fig.path': f'{self.figpath}/'},
        ...
        outputfilename = os.path.join(self.outputdir, f'{self.testtitle}.html')
        self.assertTrue(
            os.path.exists(outputfilename),
            f'File {outputfilename} was not created.',
        )
        ...
        images = glob.glob(f'{imagesdir}/*')
        logging.debug(images)
        self.assertTrue(os.path.exists(imagefile), 'image correctly named.')

        self.assertTrue(
            len(images) == 1,
            f"Contents of images dir is not correct: {','.join(images)}",
        )
    ...
    def testKnitrSettings2(self):
        ...
        imagefile = f'{os.path.join(imagesdir, os.path.splitext(os.path.split(self.contentfile)[1])[0])}-1-1.png'
    ...
    def testKnitrSettings3(self):
        ...
        imagefile = f'{os.path.join(imagesdir, os.path.splitext(os.path.split(self.contentfile)[1])[0])}-unnamed-chunk-1-1.png'
